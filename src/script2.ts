let vDOM : todo[]= []; // Our initial vDOM is an empty array
  ...
  if (parentElement !== null){
    ...
    vDOM.forEach(function(item) {
      ...
      if (existingChild) {
        ...
        childElement.dataset.id = String(item.id); // Store the ID on the element for future lookups
...
function updateVirtualDom(data :todo[]) {
    vDOM = data.map(item => {
        return {
          id: item.id,
          title: item.title,
